...
package org.opendaylight.aaa.shiro.authenticators;
 ...
 * @author liemmn
 */
public class HttpBearerAuth implements TokenAuth {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(HttpBearerAuth.class);

    private final CredentialAuth<PasswordCredentials> credentialAuth;

    public HttpBearerAuth(CredentialAuth<PasswordCredentials> credentialAuth) {
    ...
    private static String[] extractCredentialArray(final String authHeader) {
        return new String(Base64.getDecoder().decode(authHeader.substring(BEARER_PREFIX.length())),
