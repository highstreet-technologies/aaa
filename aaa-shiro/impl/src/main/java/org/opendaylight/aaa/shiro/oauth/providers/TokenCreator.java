...
public class TokenCreator {
    ...
    public String getBearerToken(HttpHeaders headers) {
        return this.getBearerToken(headers, false);
    }

    public String getBearerToken(HttpHeaders headers, boolean checkCookie) {
        final List<String> authHeader = headers.getRequestHeader("Authorization");
        if ((authHeader == null || authHeader.size()<=0 || !authHeader.get(0).startsWith("Bearer")) && checkCookie) {
            if(headers!=null) {
                Map<String, Cookie> cookies = headers.getCookies();
                Cookie ocookie=null;
                if (cookies != null) {
                    ocookie = cookies.getOrDefault(COOKIE_NAME_AUTH,null);
                }

                return ocookie == null?null : ocookie.getValue();
            }
        }
        return authHeader.get(0).substring(7);
    }

    public UserTokenPayload decode(HttpHeaders headers) throws JWTDecodeException {
        final String token = this.getBearerToken(headers);
    ...
    public NewCookie createAuthCookie(BearerToken data) {
        StringBuilder sb = new StringBuilder();
        sb.append(data.getToken());
        sb.append(String.format(";Max-Age=%d",this.tokenLifetimeSeconds));
        sb.append(";Path=/");
        sb.append(";HttpOnly");
        sb.append(";Secure");
        return new NewCookie(COOKIE_NAME_AUTH, sb.toString());
    }
}
